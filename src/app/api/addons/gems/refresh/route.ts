...
import { loadGemPriceData } from '@/app/addons/gems/lib/gemPriceApi';
import { loadGemPrices } from '@/app/addons/gems/lib/gemPriceServer';
...
export async function GET() {
    try {
        const data = await loadGemPrices();
...
export async function POST(req: NextRequest) {
    try {
        ...
        const data = await loadGemPriceData(apiKey);
        return NextResponse.json({ data });
    } catch (error) {
        ...
        if (message === 'API_KEY_REQUIRED') {
            return NextResponse.json({ code: 'API_KEY_REQUIRED', error: '로스트아크 API 키 등록이 필요합니다.' }, { status: 403 });
        }
        if (message === 'MEMBER_NOT_FOUND') {
            return NextResponse.json({ code: 'MEMBER_NOT_FOUND', error: '사용자 정보를 찾을 수 없습니다.' }, { status: 404 });
        }
        if (axiosErrorStatus(error) === 401) {
            return NextResponse.json({ code: 'INVALID_API_KEY', error: '등록된 API 키가 유효하지 않습니다.' }, { status: 502 });
        }
        console.error('Failed to refresh gem prices', error);
        return NextResponse.json({ code: 'GEM_PRICE_REFRESH_FAILED', error: '보석 시세를 갱신하지 못했습니다.' }, { status: 502 });
    }
}
