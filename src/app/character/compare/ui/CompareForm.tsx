...
function Stats({ leftInfo, rightInfo, isMobile }: CharacterProps) {
    const leftCharacter = toExpeditionCharacter(leftInfo);
    const rightCharacter = toExpeditionCharacter(rightInfo);
    const statDiffRows = getStatDiffRows(leftCharacter, rightCharacter);
    const hasStatDiff = statDiffRows.some((row) => row.leftText || row.rightText);
    const canCompareStat = Boolean(leftCharacter && rightCharacter);
    const leftDiffTexts = statDiffRows.filter((row) => row.leftText);
    const rightDiffTexts = statDiffRows.filter((row) => row.rightText);
                ...
                <div>
                    ...
                    <div className="mt-3 grid w-full grid-cols-2 gap-2 text-sm mb-2 min-[1257px]:mb-0">
                        ...
                        {hasStatDiff ? (
                            ...
                            <p className="col-span-2 text-center fadedtext">특성 수치가 동일합니다.</p>
                        )}
                    </div>
                </div>
                <Card radius="sm" shadow="sm">
